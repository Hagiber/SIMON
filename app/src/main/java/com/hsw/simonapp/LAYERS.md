...
uses `com.hsw.simonapp.engine.api.EmbeddedEngine`. Hosts that provide custom
game content may also pass a `com.hsw.simonapp.engine.game.GameDefinition`,
but must not import engine implementation packages such as
`com.hsw.simonapp.engine.render.*` or the renderer bridge package
...
- Host code may import `com.hsw.simonapp.engine.api.*`.
- Custom game bootstrap code may additionally import
  `com.hsw.simonapp.engine.game.*` to provide a `GameDefinition`.
- Host code must not import engine implementation packages such as
  `com.hsw.simonapp.engine.render.*` or renderer bridge package
  `com.hsw.vulkanrenderingengine.bridge.*`.
...
- Import boundary check: host Java sources must have no matches for
  engine implementation package imports outside `com.hsw.simonapp.engine.api.*`.
- Unit boundary check:
